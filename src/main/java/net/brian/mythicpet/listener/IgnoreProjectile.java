            ...
            if(event.getHitEntity() != null){
                if(PetUtils.isPet(event.getHitEntity())){
                    ...
                    if(projectile.getShooter() instanceof Player){
                        if(!set.testState(null,MythicPet.inst().worldGuardFlag.pet_damaged_by_player)){
                            event.setCancelled(true);
                        } else if (((Player) projectile.getShooter()).getUniqueId().equals(PetUtils.getOwner(event.getHitEntity()).get())) {
                            event.setCancelled(true);
                        } else if(!set.testState(null,MythicPet.inst().worldGuardFlag.pet_hurt)){
                            event.setCancelled(true);
                        }
                    }
                        ...
                        if(PetUtils.isPet(entity)){
                            if(!set.testState(null,MythicPet.inst().worldGuardFlag.pet_damage_pet)){
                                event.setCancelled(true);
                            }
                        }
                    }
